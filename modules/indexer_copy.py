...
class ScholarIndexer:
    ...
    def _filter_top_papers(self, papers, top_n=10):
        ...
        Returns a deduplicated list of paper IDs.
        """
        if not papers:
            return []
        ...
        # Combine and deduplicate by paperId
        paper_ids = set()
    ...
    def update_professor_papers(self, author_id, db, author_name=None):
        """Fetch and merge papers for a given authorId, then filter to top 10 recent + top 10 cited."""
        paper_ids = set(db["professors"].get(author_id, {}).get("paperIds", []))

        try:
            # Fetch up to 120 papers from Semantic Scholar
        ...
        # Store all papers in the database
        all_papers = []
        for p in papers:
            pid = p.get("paperId")
            if not pid:
        ...
        filtered_ids = self._filter_top_papers(all_papers, top_n=10)
        
        print(f"  Linked {len(filtered_ids)} papers for author {author_id} (filtered from {len(all_papers)})")
        return db, filtered_ids
